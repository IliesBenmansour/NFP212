package TP11.src.menu;

import java.util.ArrayList;
import java.util.List;

import TP11.src.menu.commande.CommandeNOP;

/**
 * Définition de menus textuels avec les entrées non sélectionnables
 * désactivées.
 *
...
public class Menu {
	...
	/**
	 * Afficher le menu. Les numéros des commandes non exécutables ne sont pas
	 * affichés.
	 */
	...
	/**
	 * Saisir le choix de l'utilisateur. Le choix correspond à une entrée du menu (y
	 * compris 0 pour quitter). L'utilisateur peut sélectionner une entrée dont la
	...
	public void selectionner() {
		...
		do {
			...
			choix_valide = 0 <= choix && choix <= this.getNbEntrees();
			if (!choix_valide) {
				System.out.println("Le numéro doit être compris entre " + 0 + " et " + this.getNbEntrees() + " !");
			}
		} while (!choix_valide);
	...
	/**
	 * Valider la sélection. Ceci consiste à exécuter la commande associée à
	 * l'entrée sélectionnée. Si l'entrée sélectionnée est non exécutable, un
	 * message d'erreur est signalé.
	 */
	public void valider() {
